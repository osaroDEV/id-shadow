...
export default function SummaryPage() {
  const { businessType, contactDetails, members } = useOnboardingStore();

  const handleEdit = (section: string) => {
    console.log(`Edit ${section} clicked`);
  };

  const getFullContactAddress = (contactDetails: ContactDetails) => {
    const parts = [
  ...
  const getFullMemberAddress = (member: (typeof members)[0]) => {
    const parts = [
      member.address,
      member.city,
      member.state,
        ...
        <div className='flex-[2] space-y-6'>
          <Card className='w-full h-auto p-8 bg-white border border-neutral-200 rounded-xl flex lg:flex-col gap-8'>
            ...
            <div className='w-full h-auto flex flex-col gap-2'>
              ...
              <Card className='w-full h-[12.5rem] p-6 sm:p-6 shadow-sm bg-neutral-50 border border-neutral-200 flex flex-col gap-8'>
                <div className='flex gap-4 sm:gap-6'>
                  ...
                  <div className='flex flex-col w-[48%]'>
                    <p className='text-sm text-gray-500'>Company Name 2</p>
                    <p className='font-medium mt-1'>
                      {businessType.alternateName}
                    </p>
                  </div>
                </div>
                <div className='flex gap-4 sm:gap-6'>
              ...
              <Card className='w-full h-auto p-6 sm:p-6 shadow-sm bg-neutral-50 border border-neutral-200 flex flex-col gap-8'>
                <div className='flex gap-4 sm:gap-6'>
                  ...
                  <div className='flex flex-col w-[48%]'>
                    <p className='text-sm text-gray-500'>Contact Address</p>
                    <p className='font-medium mt-1'>
                      {getFullContactAddress(contactDetails)}
                    </p>
                  </div>
                </div>
                <div className='flex gap-4 sm:gap-6'>
                  <div className='flex flex-col w-[48%]'>
                    ...
                    <p className='font-medium mt-1'>{contactDetails.phone}</p>
                  </div>
                  <div className='flex flex-col w-[48%]'>
                    <p className='text-sm text-gray-500'>Email Address</p>
                    <p className='font-medium mt-1'>{contactDetails.email}</p>
                  </div>
                </div>
              </Card>
            </div>
            ...
            <div className='w-full h-auto flex flex-col gap-2'>
              ...
              <Card className='w-full h-auto p-6 sm:p-6 shadow-sm bg-neutral-50 border border-neutral-200 flex flex-col gap-8'>
                {members.map((member) => (
                  <div key={member.id} className='flex flex-col bg-white rounded-base gap-5 p-6'>
                    <div className='w-full flex flex-col'>
                      ...
                      <div className='flex flex-row items-center justify-between h-[3.125rem]'>
                        ...
                        <div className='flex flex-col w-[32%] h-full text-sm'>
                          ...
                          <p className='font-medium mt-1 text-neutral-900'>
                            {(member.equity[member.equity.length - 1] === '%') ? `${member.equity}` : `${member.equity}%`}
                          </p>
                        </div>
                        <div className='flex flex-col w-[32%] h-full text-sm'>
                          <p className='text-gray-500'>Address</p>
                          <p className='font-medium mt-1 text-neutral-900'>
                          {getFullMemberAddress(member)}
                          </p>
                        </div>
                      </div>
                    </div>
